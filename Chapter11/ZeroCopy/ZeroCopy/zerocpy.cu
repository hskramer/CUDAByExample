...
const int blocksPerGrid = imin(64, (N + threadsPerBlock - 1) / threadsPerBlock);
...
float malloc_test(int size)
{
	...
	HANDLE_ERROR(cudaMalloc((void**)&d_a, size * sizeof(float)));
	HANDLE_ERROR(cudaMalloc((void**)&d_b, size * sizeof(float)));
	...
	HANDLE_ERROR(cudaMemcpy(d_a, a, size * sizeof(float), cudaMemcpyHostToDevice));
	HANDLE_ERROR(cudaMemcpy(d_b, b, size * sizeof(float), cudaMemcpyHostToDevice));
	...
	HANDLE_ERROR(cudaFree(d_partial_c));
